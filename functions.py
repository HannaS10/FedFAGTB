...
class FAGTB(object):
    ...
    def fit(self, X, y, sensitive, LAMBDA, Xtest, yt, sensitivet):
        ...
        # Einblenden der sensitiven Werte für y2
        y2 = np.expand_dims(sensitive, axis=1)

        lfadv = 0

        # Initialiserung von den prognostizierten Werten durch Auffüllen der Arrays y_pred2, y_pred und y_predt mit log(y/(1-y)) = log(odds)
        # t könnte Pseudo-Residuenberechnung sein
        ...
        with graph.as_default():
            ...
            # https://www.linkedin.com/pulse/neural-network-step-explanation-code-tensorflow-angela-ju Initalisierung NN
            ...
            loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=y_input,
                                                                         logits=logit,
                                                                         name='loss'))  # Berechnet die sigmoidale Kreuzentropie bei gegebenen Logits.
            train_steps = tf.train.GradientDescentOptimizer(learning_rate2).minimize(
                loss)  # Skalierung der Lernrate (?) / Berechnung Output Values for new tree
        ...
        # für jeden Schätzer (Baum) führe folgendes durch:
        for i in range(self.n_estimators):

            y_pred2 = np.expand_dims(1 / (1 + np.exp(-y_pred)),
                                     axis=1)  # y_pred2 (prognostizierte Werte) Array wird um Spalte 1/(1+e⁻y_pred) ergänzt

            train_feed_dict = {X_input: y_pred2, y_input: y2}
            sess.run(train_steps,
                     feed_dict=train_feed_dict)  # minimiere Ableitung (GradientDescentOptimizer) mit geg. Lernrate und Verlustfkt.
            cur_loss = sess.run(loss, feed_dict=train_feed_dict)  # berechnet den aktuellen Verlust
            train_acc = sess.run(acc, feed_dict=train_feed_dict)  # berechnet die Trainings-Genauigkeit
            S_ADV = sess.run(sigm2, feed_dict=train_feed_dict)  # berechnet den gegnerischen (?)

            gradient_adv = sess.run(var_grad, feed_dict=train_feed_dict)  # Bestimmung gegnerischer Gradient
            ...
            t = -np.squeeze(lfadv.T)  # Entfernen der Achse lfadv.T
            proj = 0
            gradient = y - 1 / (1 + np.exp(-y_pred)) - LAMBDA * t - proj  # Ableitung des Trainingsverlustes
            self.trees[i].fit(X,
                              gradient)  # Anpassung eines Klassifikators an Pseudo-Residuen unter Verwendung der Trainingsmenge {(xi, uim)}n
            update = self.trees[i].predict(X)  # Hinzufügen eines neuen Baumes (training)

            y_pred += np.multiply(self.learning_rate, update)  # Aktualisierung des Modells (training)
            y_fin = 1 / (1 + np.exp(-y_pred))  # neue vorhergesagte Werte (training)

            losstraining = lossgr(y,
                                  y_fin)  # Hinzufügen eines neuen Baumes zu Ensemble: - y * np.log(y_fin) - (1 - y) * np.log(1 - y_fin)
            lossglobal = losstraining - LAMBDA * t  # Ableitung glob. Traingsverlust (training)

            updatet = self.trees[i].predict(Xtest)  # NHinzufügen eines neuen Baumes (test)
            y_predt += np.multiply(self.learning_rate, updatet)  # Update learning model (test)
            y_predt2 = 1 / (1 + np.exp(-y_predt))  # neue vorhergesagte Werte (test)
            accuracy = accuracy_score(y, np.squeeze(y_fin) > 0.5)  # Berechnung Genauigkeit Training
            accuracyt = accuracy_score(yt, np.squeeze(y_predt2) > 0.5)  # Berechnung Genauigkeit Test

            if i % 5 == 0:
                print(i, np.sum(lfadv), np.sum(losstraining), np.sum(lossglobal), "Accuracy:", round(accuracy, 4),
                      " test : ", round(accuracyt, 4), " Prule Train : ", p_rule(y_fin, sensitive) / 100,
                      " Prule test : ", p_rule(y_predt2, sensitivet) / 100)
            table = np.vstack(
                [table, [accuracy, accuracyt, p_rule(y_fin, sensitive) / 100, p_rule(y_predt2, sensitivet) / 100]])
        return {'y_pred2': y_pred2,
                'S_ADV': S_ADV}  # Output einer Iteration: neuer Vorhersagewert y_pred2 und neuer Angriffsvorhersagewert S_ADV
